...
| `--hungarian` | 关闭 | 启用匈牙利算法（将会极大提升Retrieval分数） |
| `--epoch` | `200` | 训练轮数 |
...
1. **EEG 编码器训练**（80 轮）：`EEGProjectLayer` 将 EEG [63 通道 × 250 时间步] 映射为 1024 维 CLIP 空间。同时以图像、文本、深度图、边缘图四种模态嵌入为对比学习目标。代码支持不确定性感知的模态掩码作为可选训练模式，但本文报告的多种子结果使用默认联合训练，并未启用该 flag。
