...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("run_modified.log", mode="w", encoding="utf-8"),  # 输出到文件 run.log
...
if __name__ == "__main__":
    ...
    data_root = Path("./true-image-eeg-data")
    ...
    logging.info("\n" + "=" * 60)
    logging.info("开始训练 ATM_S 模型（10次重复实验）")
    logging.info("=" * 60)
    ...
    for run in range(10):
        ...
        # 重新初始化模型（使用正确的参数）
        model = ATM_S(
            num_channels=63,      # 你的 EEG 数据有 63 个通道
            time_len=250,         # 每个 trial 有 250 个时间点
            out_dim=clip_dim,     # clip_dim 应该是 1024
            dropout=0.1
        ).to(device)
        ...
        all_top1.append(retrieval_metrics['eeg_to_image_top1'])
        all_top5.append(retrieval_metrics['eeg_to_image_top5'])

    # 计算统计结果
    import numpy as np
    top1_mean, top1_std = np.mean(all_top1), np.std(all_top1)
    top5_mean, top5_std = np.mean(all_top5), np.std(all_top5)

    logging.info(f"\n{'='*60}")
    logging.info(f"10次实验结果汇总:")
    logging.info(f"Top-1: 均值={top1_mean:.2f}%, 标准差={top1_std:.2f}")
    logging.info(f"Top-5: 均值={top5_mean:.2f}%, 标准差={top5_std:.2f}")
    logging.info(f"原始数据 - Top1: {all_top1}")
    logging.info(f"原始数据 - Top5: {all_top5}")
    logging.info(f"{'='*60}")