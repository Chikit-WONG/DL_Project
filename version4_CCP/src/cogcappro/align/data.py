...
def load_diffusion_embeddings(embedding_path: str, img_paths: list[str]) -> dict[str, torch.Tensor]:
    ...
    global_fallback = {
        mod: full_embeddings[mod][next(iter(full_embeddings[mod]))] if full_embeddings[mod] else None
    ...
    for img_path in img_paths:
        ...
        for mod in modalities:
            if embed_key in full_embeddings[mod]:
                h_embeddings[mod].append(full_embeddings[mod][embed_key])
            elif img_filename in full_embeddings[mod]:
                # Backward compatibility with old basename-keyed embeddings
                h_embeddings[mod].append(full_embeddings[mod][img_filename])
            elif global_fallback[mod] is not None:
                h_embeddings[mod].append(global_fallback[mod])
            else:
                raise ValueError(f"Embedding not found for {mod} modality of {embed_key} in {embedding_path}")
