...
class IPAdapterGenerator:
    ...
    def _init_pipeline(self, sd_path, ip_adapter_path):
        ...
        # Enable VAE upcast so the pipeline temporarily decodes in float32,
        # preventing float16 overflow → NaN → all-black identical images.
    ...
    def _prepare_embeddings(self, embeds_dict):
        # When guidance_scale <= 1.0, do_classifier_free_guidance=False in the pipeline.
        # The pipeline's prepare_ip_adapter_image_embeds then passes embeds as-is (no chunk(2)),
        # so we must provide only the conditional embed [1, tokens, dim], not the [2, tokens, dim]
        # uncond+cond stack — the latter gets misinterpreted as batch=2 in the attention processor.
        ...
        for mod in self.modalities:
            ...
            if do_cfg:
                ...
                ip_embeds.append(torch.stack([uncond_embed, embed], dim=0))  # [2, 1, 1024]
            else:
                ip_embeds.append(embed.unsqueeze(0))  # [1, 1, 1024] — pipeline requires ≥3D
        return ip_embeds
    ...
    def _image_to_embedding(self, image):
        if isinstance(image, (str, PIL.Image.Image)):
            ...
            raise ValueError("Input image must be a file path, a PIL.Image object, or a list of them")

        resolved_images = []
        for item in image_inputs:
            if isinstance(item, str):
                resolved_images.append(load_image(item))
            elif isinstance(item, PIL.Image.Image):
                resolved_images.append(item)
            else:
                raise ValueError("Each input image must be a file path or a PIL.Image object")

        if len(resolved_images) == 1 and self.num_modalities > 1:
            # The pipeline expects one conditioning image per loaded IP-Adapter.
            resolved_images = resolved_images * self.num_modalities

        return self.pipe.prepare_ip_adapter_image_embeds(
            ip_adapter_image=resolved_images,
...
def prepare_embedding(
    ...
    for modality in valid_modalities:
        ...
        image_paths = []
        for root, _, files in os.walk(modal_dir, followlinks=True):
        ...
        for img_path in tqdm(image_paths, desc=f"Processing {modality} modality"):
            try:
                ...
                embed_key = f"{class_name}/{img_filename}"
                rel_path = os.path.relpath(img_path, modal_dir)
                if rel_path.startswith("train_images") or rel_path.startswith("training_images"):
                ...
                target_dict[embed_key] = valid_embedding
