...
def run(args: argparse.Namespace):
    ...
    num_devices = len(config["devices"]) if accelerator == "gpu" else 1
    trainer_devices = config["devices"] if accelerator == "gpu" else 1
    trainer_strategy = DDPStrategy(find_unused_parameters=True) if num_devices > 1 else "auto"
