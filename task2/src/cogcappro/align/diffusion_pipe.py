...
class SimpleAlignMLP(nn.Module):
    ...
    def forward(self, c: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        # No output normalization — let the loss handle alignment
        return out
...
class SDEmbeddingLoss(nn.Module):
    """
    Alignment loss: MSE + cosine on unit-normalized embeddings.
    Both pred and target are normalized before MSE so scale differences
    cannot cause mode collapse.
    """
    ...
    def forward(self, u: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            u: Model output  (B, D)
            v: Target ground truth  (B, D)
        Returns:
            Scalar loss
        """
        # Normalize both to unit sphere so MSE measures directional error
        u_norm = F.normalize(u, p=2, dim=-1)
        v_norm = F.normalize(v, p=2, dim=-1)
        # MSE on unit-normalized embeddings
        loss_mse = F.mse_loss(u_norm, v_norm)
        # Cosine loss reinforces directional alignment
        loss_cos = (1 - F.cosine_similarity(u_norm, v_norm, dim=-1)).mean()
        return self.w_mse * loss_mse + self.w_cos * loss_cos
