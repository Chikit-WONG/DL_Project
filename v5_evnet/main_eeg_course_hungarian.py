...
def get_dataset(base_path, feature_path, sub, select_channels, use_filter=False,
    ...
    test_evnet = None
    if use_evnet:
        evnet_train_path = os.path.join(feature_path, 'EVNet_RN50_train.pt')
        evnet_test_path = os.path.join(feature_path, 'EVNet_RN50_test.pt')
...
def get_test_accu(model, params, test_dataloader, device, hungarian=False):
    ...
    # 拼接全部样本（val/test loader 本来就是整批）
    tfea = torch.cat(all_tfea, dim=0)   # [N, D]
    embed = torch.cat(all_embed, dim=0)  # [N, D]
    sim = tfea @ embed.T                  # [N, N]
...
def train(params, logger):
    ...
    logger.info(f"Using EVNet: {params['use_evnet']}")
    ...
    for e in range(params['epoch']):
        ...
        # 选择标准：Hungarian 模式用 hung_top1 做选择；否则用标准 top1
        ...
        if e == params['epoch'] - 1:
            saved_metric.update({
                'final_test_top1_acc': test_res['top1'],
                'final_test_top3_acc': test_res['top3'],
                'final_test_top5_acc': test_res['top5'],
            })
            if use_hungarian:
                saved_metric.update({
                    'final_test_hung_top1_acc': test_res['hung_top1'],
                    'final_test_hung_top5_acc': test_res['hung_top5'],
                })
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_evnet',        action='store_true', help='Use EVNet preprocessed features', default=True)
    parser.add_argument('--hungarian',        action='store_true', help='Use Hungarian algorithm for evaluation and model selection', default=False)
    parser.add_argument('--blur_level',       type=int, nargs='+',
                        default=[1, 3, 15, 21, 33, 45, 57, 63],
                        help='Blur levels to use (default: 1 3 15 21 33 45 57 63)')
    ...
    BLUR_LEVELS = [f'l_{x}' for x in args.blur_level]

    save_path = os.path.join(args.output_dir, args.net_name, 
                              f"{'EVNet_' if args.use_evnet else ''}{time.strftime('%Y-%m-%d-%H-%M')}")
    ...
    logger.info(f"EVNet enabled: {args.use_evnet}")
    ...
    # Summary — three columns: val-selected / best test / final epoch
    ...
    top1_final = [m['final_test_top1_acc'] for m in all_metrics]
    top5_final = [m['final_test_top5_acc'] for m in all_metrics]

    def _stat(vals):
        return f"{np.mean(vals):.4f} ± {np.std(vals):.4f}"

    logger.info('=' * 60)
    logger.info(f'Results (EVNet={args.use_evnet}, Hungarian={args.hungarian}):')
    logger.info(f'  {"":>20s}  {"val-sel":>22s}  {"best-test":>22s}  {"final-epoch":>22s}')
    logger.info(f'  {"Top-1":>20s}  {_stat(top1_sel):>22s}  {_stat(top1_best):>22s}  {_stat(top1_final):>22s}')
    logger.info(f'  {"Top-5":>20s}  {_stat(top5_sel):>22s}  {_stat(top5_best):>22s}  {_stat(top5_final):>22s}')
    ...
    if args.hungarian:
        ...
        hung_top1_final = [m.get('final_test_hung_top1_acc', np.nan) for m in all_metrics]
        hung_top5_final = [m.get('final_test_hung_top5_acc', np.nan) for m in all_metrics]
        logger.info(f'  {"Hun-Top-1":>20s}  {_stat(hung_top1_sel):>22s}  {_stat(hung_top1_best):>22s}  {_stat(hung_top1_final):>22s}')
        logger.info(f'  {"Hun-Top-5":>20s}  {_stat(hung_top5_sel):>22s}  {_stat(hung_top5_best):>22s}  {_stat(hung_top5_final):>22s}')
