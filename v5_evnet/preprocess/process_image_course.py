...
class Make_dataset(nn.Module):
    def __init__(self, checkpoint_path, batch_size=128, use_evnet=True):
        super().__init__()
        ...
        if self.use_evnet:
            print("Initializing EVNet frontend...")
            ...
            print("EVNet frontend initialized successfully.")
        else:
...
if __name__ == '__main__':
    ...
    # 如果需要单独的 EVNet 特征文件
    train_evnet_save = os.path.join(args.save_dir, "EVNet_RN50_train.pt")
    test_evnet_save = os.path.join(args.save_dir, "EVNet_RN50_test.pt")

    print(f"Using RN50 checkpoint: {args.clip_checkpoint}")
    print(f"Saving image features to: {args.save_dir}")
    print(f"EVNet enabled: {not args.no_evnet and EVNET_AVAILABLE}")
    
    # 创建模型实例
    model = Make_dataset(args.clip_checkpoint, args.batch_size, use_evnet=(not args.no_evnet))
