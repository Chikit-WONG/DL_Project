...
from models.data_bridge import detect_data_format, load_pt_data
...
model_type = 'ViT-H-14'
vlmodel, preprocess_train, feature_extractor = create_open_clip_vit_h_14(
    device=device,
    precision='fp32',
)
...
class EEGDataset():
    ...
    def __init__(self, data_path, exclude_subject=None, subjects=None, train=True, time_window=[0, 1.0], classes = None, pictures = None, val_size=None):
        ...
        self.train = train
        self.data_format = detect_data_format(data_path)
        self.subject_list = os.listdir(data_path) if self.data_format == 'paper_subject_dirs' else ['sub-01']
        ...
        if self.data_format == 'paper_subject_dirs':
        ...
        if self.classes is None and self.pictures is None:
            ...
            features_filename = get_feature_cache_path(
                feature_root=features_path,
    ...
    def load_data(self):
        if self.data_format == 'pt_single_subject':
            split = 'train' if self.train else 'test'
            image_dir = img_directory_training if self.train else img_directory_test
            loaded = load_pt_data(
                data_path=self.data_path,
                split=split,
                avg_trials=True,  # course requires True for test; True recommended for train
                image_dir=image_dir,
                classes=self.classes,
                pictures=self.pictures,
            )
            self.sample_image_indices = loaded['sample_image_indices']
            self.sample_text_indices = loaded['sample_text_indices']
            self.times = loaded['times']
            self.ch_names = loaded['channel_names']
            data_tensor = loaded['eeg']
            label_tensor = loaded['labels']
            texts = loaded['texts']
            images = loaded['images']
            print(f"PT-format dataset loaded: data {tuple(data_tensor.shape)}, labels {tuple(label_tensor.shape)}")
    ...
    def extract_eeg(self, eeg_data, time_window):
        if self.times is None:
            return eeg_data
        if len(self.times) != eeg_data.shape[-1]:
            print(
                f"Skipping time-window extraction because times has length {len(self.times)} "
                f"but EEG has {eeg_data.shape[-1]} samples."
            )
            return eeg_data
    ...
    def __getitem__(self, index):
        ...
        if self.data_format == 'pt_single_subject':
            text_index = self.sample_text_indices[index]
            img_index = self.sample_image_indices[index]
