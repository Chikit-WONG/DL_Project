...
def _avg_trials_for_split(split: Literal["train", "test"]) -> bool:
    """训练与测试均对多试次 EEG 直接取平均（mean），每图一条样本；训练集约 16540 条。"""
...
class EEGProjectDataset(Dataset):
    ...
    def __init__(self, data_directory: Union[str, Path], split: str = "train",
        ...
        avg_trials = _avg_trials_for_split(split)  # train/test: True，试次维 mean
        ...
        logging.info(
            f"EEGProjectDataset split={split} -> avg_trials={avg_trials} (训练/测试均对试次取平均)"
        )
...
def train_atm(model, train_loader, epochs=40, lr=3e-4, device="cuda",
              checkpoint_path: Optional[Union[str, Path]] = None, test_loader=None):
    device_obj = torch.device(device) if not isinstance(device, torch.device) else device
    model.to(device_obj)
    ...
    for epoch in range(epochs):
        ...
        # 每个epoch后进行测试
        epoch_metrics = {"epoch": epoch + 1, "loss": avg_loss, "lr": current_lr}
        if test_loader is not None:
            test_metrics = evaluate_retrieval(model, test_loader, device=device_obj)
            epoch_metrics.update(test_metrics)
            logging.info(f"Epoch [{epoch+1}/{epochs}], Loss: {avg_loss:.4f}, LR: {current_lr:.6f} | "
                        f"Top1: {test_metrics['eeg_to_image_top1']:.2f}% | Top5: {test_metrics['eeg_to_image_top5']:.2f}%")
        else:
            logging.info(f"Epoch [{epoch+1}/{epochs}], Loss: {avg_loss:.4f}, LR: {current_lr:.6f}")
